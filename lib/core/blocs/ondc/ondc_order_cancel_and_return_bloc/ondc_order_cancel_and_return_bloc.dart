...
class ONDCOrderCancelAndReturnReasonsBloc
    extends Bloc<ONDCOrderCancelAndReturnEvent, ONDCOrderCancelAndReturnState>
    with LogMixin {
  ...
  ONDCOrderCancelAndReturnReasonsBloc({
    ...
    on<LoadReasonsForReturnEvent>((event, emit) async {
      ...
      try {
        ...
        orderId = event.orderId;
        orderNumber = event.orderNumber;
        emit(ReasonsLoadedForReturnState(
            reasons: reasons,
            orderId: event.orderId,
            orderNumber: event.orderNumber));
      } catch (e) {
    ...
    on<CancelFullOrderRequestEvent>((event, emit) async {
      ...
      try {
        String status = await orderCancelRepository.fullOrderCancelPost(
            code: selectedCode, orderId: orderId);
    ...
    on<SelectedCodeForReturnEvent>((event, emit) {
      if (event.code != "null" || event.code != "") {
        ...
        BlocProvider.of<UploadImageAndReturnRequestCubit>(event.context)
            .getPrerequisiteData(
                orderId: orderId,
                orderNumber: orderNumber,
