...
class _ONDCOrderDetailsScreenState extends State<ONDCOrderDetailsScreen> {
  ...
  Widget _getGroupSeparator(PreviewWidgetOndcItem element,
      SingleOrderModel orderDetails, String? fulfillmentWidget, int countNum) {
      ...
      if (orderDetails.quotes!.first.tracks!.isNotEmpty) {
        for (var track in orderDetails.quotes!.first.tracks!) {
          if (track.fulfillmentId.toString() ==
            ...
            if ((track.url).toString() != "" && track.url != null) {
              return true;
            }
          }
        }
  ...
  body(context, Data orderDetails) {
    ...
    String shopName = orderDetails.singleOrderModel!.storeLocation!.store!.name
            .toString(),
        orderId =
            orderDetails.singleOrderModel!.quotes!.first.orderId.toString(),
        orderStatus = orderDetails.singleOrderModel!.status.toString(),
        orderNumber = orderDetails.singleOrderModel!.orderNumber.toString();

    List<CartItemPrices> products = orderDetails
        .singleOrderModel!.quotes!.first.cartItemPrices as List<CartItemPrices>;

    String? paymentStatusFunction() {
      if (orderDetails.singleOrderModel!.payment!.paymentStatus != null) {
        if ((orderDetails.singleOrderModel!.payment!.paymentStatus) as bool !=
            false) {
          return "Paid";
        }
      }
    ...
    bool hasInvoice =
        orderDetails.singleOrderModel!.invoice == null ? false : true;
    print("##############################################################3"
    ...
    isAllCancellable({required CartItemPrices element}) {
      if (element.cancellable != null &&
        ...
        for (var track
            in orderDetails.singleOrderModel!.quotes!.first.tracks!) {
          if (track.fulfillmentId.toString() ==
              ...
              return InkWell(
                onTap: () {
                  ...
                  BlocProvider.of<ONDCOrderCancelAndReturnReasonsBloc>(context)
                      .add(LoadReasonsForPartialOrderCancelEvent(
                          orderId: orderDetails
                              .singleOrderModel!.quotes!.first.orderId
                              .toString(),
                          orderNumber: orderDetails
                              .singleOrderModel!.orderNumber
                              .toString(),
            ...
            if ((track.state).toString() == "Delivered" ||
              ...
              return InkWell(
                onTap: () {
                  ...
                  BlocProvider.of<ONDCOrderCancelAndReturnReasonsBloc>(context)
                      .add(LoadReasonsForReturnEvent(
                          orderId: orderId,
                          orderNumber: orderNumber,
                          product: element));
                },
                child: Text("Return",
                    style: FontStyleManager().s14fw700GreyUnderLne),
              );
            } else {
              return null;
            }
    ...
    return ListView(children: [
      ...
      OrderDetailsTable(
        ...
        invoiceUrl:
            hasInvoice ? orderDetails.singleOrderModel!.invoice.toString() : "",
      ),
      Padding(
                  ...
                  Center(
                    child: Padding(
                      ...
                      child: SizedBox(
                        width: 50,
                        child: AutoSizeText(
                          "₹${priceFormatter(value: previewWidgetModel.previewWidgetModel.price.toString())}",
                          maxFontSize: 16,
                          minFontSize: 10,
      ...
      InvoiceTable(
        ...
        totalPrice:
            orderDetails.singleOrderModel!.quotes!.first.totalPrice.toString(),
      ),
